  ...
  return (
    <div style={{ position: "fixed", inset: 0, zIndex: 2000, background: "rgba(0,0,0,0.96)", display: "flex", alignItems: "center", justifyContent: "center", padding: 16 }} onClick={onClose}>
      <div style={{ background: "#08070f", border: "1px solid rgba(123,47,255,0.25)", borderRadius: 20, width: "100%", maxWidth: 720, maxHeight: "92vh", overflow: "hidden", display: "flex", flexDirection: "column", boxShadow: "0 32px 80px rgba(0,0,0,0.9), 0 0 60px rgba(123,47,255,0.15)", position: "relative" }} onClick={e => e.stopPropagation()}>
          ...
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
            <div>
              <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 6 }}>
                ...
                <div>
                  <div style={{ fontFamily: "'Rajdhani',sans-serif", fontWeight: 700, fontSize: 22, color: "#fff", textTransform: "uppercase", letterSpacing: "0.5px" }}>Tournament Leaderboard</div>
                  <div style={{ fontSize: 12, color: "#7755aa", fontFamily: "'Rajdhani',sans-serif" }}>{tournament.gameName}</div>
                </div>
              </div>
              <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
                <div style={{ display: "flex", alignItems: "center", gap: 5, padding: "4px 12px", background: "rgba(0,255,136,0.08)", border: "1px solid rgba(0,255,136,0.2)", borderRadius: 20 }}>
                  <span style={{ width: 6, height: 6, borderRadius: "50%", background: "#00FF88", animation: "livePulse 1s ease-in-out infinite" }} />
                  <span style={{ fontSize: 10, color: "#00FF88", fontFamily: "'Rajdhani',sans-serif", fontWeight: 700 }}>LIVE — updates every 10s</span>
                </div>
                <div style={{ padding: "4px 12px", background: "rgba(255,183,0,0.08)", border: "1px solid rgba(255,183,0,0.2)", borderRadius: 20, fontSize: 10, color: "#FFB700", fontFamily: "'Rajdhani',sans-serif", fontWeight: 700 }}>
                  💰 Prize Pool: {(pool).toFixed(0)} ARCADE
                </div>
                <div style={{ padding: "4px 12px", background: "rgba(123,47,255,0.08)", border: "1px solid rgba(123,47,255,0.2)", borderRadius: 20, fontSize: 10, color: "#a67fff", fontFamily: "'Rajdhani',sans-serif", fontWeight: 700 }}>
                  👥 {players.length} Players
                </div>
              </div>
            </div>
            <button onClick={onClose} style={{ background: "rgba(255,68,68,0.08)", border: "1px solid rgba(255,68,68,0.2)", borderRadius: 8, color: "#ff6b6b", padding: "8px 16px", cursor: "pointer", fontSize: 13, fontFamily: "'Rajdhani',sans-serif", fontWeight: 700, flexShrink: 0 }}>✕ Close</button>
          </div>
        </div>
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: "20px 28px" }}>
          {loading ? (
            <div style={{ padding: 48, textAlign: "center" }}>
              <div style={{ fontSize: 40, marginBottom: 12, animation: "trophyBounce 1s ease-in-out infinite" }}>⏳</div>
              <div style={{ color: "#5533aa", fontFamily: "'Rajdhani',sans-serif", fontSize: 13, letterSpacing: "1px" }}>LOADING SCORES...</div>
            </div>
          ) : players.length === 0 ? (
            <div style={{ padding: 48, textAlign: "center" }}>
              <div style={{ fontSize: 56, marginBottom: 16 }}>👾</div>
              <div style={{ fontFamily: "'Rajdhani',sans-serif", fontWeight: 700, fontSize: 18, color: "#c4a0ff", marginBottom: 8 }}>No scores yet</div>
              <div style={{ color: "#5533aa", fontFamily: "'Rajdhani',sans-serif", fontSize: 13 }}>Be the first to play and claim the top spot!</div>
            </div>
          ) : (
            <>
              {/* Top 3 Podium */}
              {players.length >= 1 && (
                <div style={{ marginBottom: 24 }}>
                  <div style={{ fontSize: 10, color: "#5533aa", fontFamily: "'Rajdhani',sans-serif", fontWeight: 700, textTransform: "uppercase", letterSpacing: "1.5px", marginBottom: 14 }}>Top Players</div>
                  <div style={{ display: "grid", gridTemplateColumns: players.length >= 3 ? "1fr 1.1fr 1fr" : players.length === 2 ? "1fr 1fr" : "1fr", gap: 10, alignItems: "flex-end", marginBottom: 4 }}>
                    {/* 2nd place */}
                    {players.length >= 2 && (
                      <div style={{ background: podiumBg[1], border: `1px solid ${podiumBorder[1]}`, borderRadius: 14, padding: "16px 14px", textAlign: "center", order: 0 }}>
                        <div style={{ fontSize: 28, marginBottom: 8 }}>🥈</div>
                        <div style={{ width: 44, height: 44, borderRadius: "50%", background: `linear-gradient(135deg,${podiumColors[1]},${podiumColors[1]}88)`, margin: "0 auto 8px", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 15, fontWeight: 700, color: "#08070f", fontFamily: "'Rajdhani',sans-serif", border: `2px solid ${podiumColors[1]}` }}>
                          {players[1].slice(2, 4).toUpperCase()}
                        </div>
                        <div style={{ fontFamily: "monospace", fontSize: 10, color: podiumColors[1], marginBottom: 4 }}>{players[1].slice(0, 8)}...</div>
                        <div style={{ fontFamily: "'Orbitron',sans-serif", fontWeight: 700, fontSize: 18, color: podiumColors[1] }}>{scores[1].toLocaleString()}</div>
                        {prizes[1] > 0 && <div style={{ fontSize: 10, color: "#FFB700", fontFamily: "'Rajdhani',sans-serif", fontWeight: 700, marginTop: 4 }}>+{prizes[1].toFixed(0)} ARCADE</div>}
                      </div>
                    )}
                    {/* 1st place */}
                    <div style={{ background: podiumBg[0], border: `2px solid ${podiumBorder[0]}`, borderRadius: 14, padding: "20px 14px", textAlign: "center", order: players.length >= 2 ? 1 : 0, boxShadow: `0 0 30px rgba(255,183,0,0.2)` }}>
                      <div style={{ fontSize: 34, marginBottom: 8 }}>🥇</div>
                      <div style={{ width: 52, height: 52, borderRadius: "50%", background: `linear-gradient(135deg,${podiumColors[0]},#ff8800)`, margin: "0 auto 8px", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 18, fontWeight: 700, color: "#08070f", fontFamily: "'Rajdhani',sans-serif", border: `3px solid ${podiumColors[0]}`, boxShadow: `0 0 20px rgba(255,183,0,0.4)` }}>
                        {players[0].slice(2, 4).toUpperCase()}
                      </div>
                      <div style={{ fontFamily: "monospace", fontSize: 10, color: podiumColors[0], marginBottom: 4 }}>{players[0].slice(0, 8)}...</div>
                      <div style={{ fontFamily: "'Orbitron',sans-serif", fontWeight: 700, fontSize: 22, color: podiumColors[0] }}>{scores[0].toLocaleString()}</div>
                      {prizes[0] > 0 && <div style={{ fontSize: 11, color: "#FFB700", fontFamily: "'Rajdhani',sans-serif", fontWeight: 700, marginTop: 4 }}>+{prizes[0].toFixed(0)} ARCADE</div>}
                    </div>
                    {/* 3rd place */}
                    {players.length >= 3 && (
                      <div style={{ background: podiumBg[2], border: `1px solid ${podiumBorder[2]}`, borderRadius: 14, padding: "16px 14px", textAlign: "center", order: 2 }}>
                        <div style={{ fontSize: 28, marginBottom: 8 }}>🥉</div>
                        <div style={{ width: 44, height: 44, borderRadius: "50%", background: `linear-gradient(135deg,${podiumColors[2]},${podiumColors[2]}88)`, margin: "0 auto 8px", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 15, fontWeight: 700, color: "#fff", fontFamily: "'Rajdhani',sans-serif", border: `2px solid ${podiumColors[2]}` }}>
                          {players[2].slice(2, 4).toUpperCase()}
                        </div>
                        <div style={{ fontFamily: "monospace", fontSize: 10, color: podiumColors[2], marginBottom: 4 }}>{players[2].slice(0, 8)}...</div>
                        <div style={{ fontFamily: "'Orbitron',sans-serif", fontWeight: 700, fontSize: 18, color: podiumColors[2] }}>{scores[2].toLocaleString()}</div>
                        {prizes[2] > 0 && <div style={{ fontSize: 10, color: "#FFB700", fontFamily: "'Rajdhani',sans-serif", fontWeight: 700, marginTop: 4 }}>+{prizes[2].toFixed(0)} ARCADE</div>}
                      </div>
                    )}
                  </div>
                </div>
              ...
              {players.length > 3 && (
                <div>
                  <div style={{ fontSize: 10, color: "#5533aa", fontFamily: "'Rajdhani',sans-serif", fontWeight: 700, textTransform: "uppercase", letterSpacing: "1.5px", marginBottom: 10 }}>All Players</div>
                  {players.map((addr, i) => (
                    <div key={addr} style={{ display: "flex", alignItems: "center", gap: 14, padding: "12px 16px", background: i < 3 ? podiumBg[i] : i % 2 === 0 ? "rgba(123,47,255,0.04)" : "transparent", borderRadius: 10, border: `1px solid ${i < 3 ? podiumBorder[i] : "rgba(123,47,255,0.08)"}`, marginBottom: 6, transition: "all 0.2s" }}
                      onMouseEnter={e => { e.currentTarget.style.background = "rgba(123,47,255,0.08)"; e.currentTarget.style.borderColor = "rgba(123,47,255,0.2)"; }}
                      onMouseLeave={e => { e.currentTarget.style.background = i < 3 ? podiumBg[i] : i % 2 === 0 ? "rgba(123,47,255,0.04)" : "transparent"; e.currentTarget.style.borderColor = i < 3 ? podiumBorder[i] : "rgba(123,47,255,0.08)"; }}
                    >
                      <div style={{ fontFamily: "'Orbitron',sans-serif", fontSize: 13, color: i < 3 ? podiumColors[i] : "#5533aa", fontWeight: 700, minWidth: 30, textAlign: "center" }}>
                        {i < 3 ? medals[i] : `#${i + 1}`}
                      </div>
                      <div style={{ width: 36, height: 36, borderRadius: "50%", background: i < 3 ? `linear-gradient(135deg,${podiumColors[i]},${podiumColors[i]}88)` : "rgba(123,47,255,0.2)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 12, fontWeight: 700, color: i < 3 ? "#08070f" : "#a67fff", fontFamily: "'Rajdhani',sans-serif", flexShrink: 0, border: `1px solid ${i < 3 ? podiumColors[i] : "rgba(123,47,255,0.3)"}` }}>
                        {addr.slice(2, 4).toUpperCase()}
                      </div>
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <div style={{ fontFamily: "monospace", fontSize: 12, color: i < 3 ? podiumColors[i] : "#9977cc" }}>{addr.slice(0, 10)}...{addr.slice(-4)}</div>
                        {i < 3 && prizes[i] > 0 && <div style={{ fontSize: 10, color: "#FFB700", fontFamily: "'Rajdhani',sans-serif", fontWeight: 700 }}>🏆 Prize: {prizes[i].toFixed(0)} ARCADE</div>}
                      </div>
                      <div style={{ fontFamily: "'Orbitron',sans-serif", fontWeight: 700, fontSize: 18, color: i < 3 ? podiumColors[i] : "#a67fff", minWidth: 80, textAlign: "right" }}>
        ...
        {/* Footer */}
        <div style={{ padding: "14px 28px", borderTop: "1px solid rgba(123,47,255,0.12)", background: "rgba(123,47,255,0.04)", flexShrink: 0 }}>
          <button onClick={() => { onClose(); navigate(`/play/${tournament.gameId}`); }} style={{ width: "100%", padding: "13px", background: "linear-gradient(135deg,#7B2FFF,#5a1fd4)", border: "none", borderRadius: 10, color: "#fff", fontSize: 13, fontWeight: 700, cursor: "pointer", fontFamily: "'Rajdhani',sans-serif", letterSpacing: "1.5px", textTransform: "uppercase" }}>
    ...
    <>
      <div onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)} style={{ background: hovered ? "rgba(14,12,26,0.95)" : "rgba(10,8,20,0.9)", border: `1px solid ${hovered ? sc.border : "rgba(123,47,255,0.15)"}`, borderRadius: 16, overflow: "hidden", transition: "all 0.3s cubic-bezier(0.4,0,0.2,1)", transform: hovered ? "translateY(-4px) scale(1.01)" : "translateY(0) scale(1)", boxShadow: hovered ? `0 20px 40px rgba(0,0,0,0.5), 0 0 30px ${sc.color}18` : "0 4px 12px rgba(0,0,0,0.3)", position: "relative" }}>
        ...
        <div style={{ padding: "14px 16px" }}>
          ...
          {/* Fix 2: Insufficient balance warning */}
          {!isJoined && !isFull && status !== "Ended" && status !== "Cancelled" && !hasEnoughBalance && address && (
            <div style={{ padding: "8px 12px", background: "rgba(255,68,68,0.06)", border: "1px solid rgba(255,68,68,0.2)", borderRadius: 8, fontSize: 11, color: "#ff6b6b", fontFamily: "'Rajdhani',sans-serif", fontWeight: 700, marginBottom: 8, textAlign: "center" }}>
              ⚠ Insufficient ARCADE —{" "}
              <span onClick={() => navigate("/marketplace")} style={{ color: "#FFB700", cursor: "pointer", textDecoration: "underline" }}>Buy more</span>
            </div>
          )}
          ...
          {status !== "Ended" && status !== "Cancelled" && (
            isJoined ? (
              <div style={{ display: "flex", flexDirection: "column", gap: 7 }}>
                ...
                {status === "Active" && (
                  <button onClick={() => navigate(`/play/${tournament.gameId}`)} style={{ width: "100%", padding: "10px", background: "linear-gradient(135deg,#7B2FFF,#5a1fd4)", border: "none", borderRadius: 8, color: "#fff", fontSize: 12, fontWeight: 700, cursor: "pointer", fontFamily: "'Rajdhani',sans-serif", letterSpacing: "1px", textTransform: "uppercase" }}>
                    🎮 Play Now
                  </button>
                )}
              </div>
...
export default function Tournaments() {
  ...
  const fetchTournaments = async () => {
    ...
    try {
      ...
      const results = await Promise.all(Array.from({ length: total }, (_, i) => publicClient.readContract({ address: TOURNAMENT_ADDRESS, abi: TOURNAMENT_ABI, functionName: "getTournamentInfo", args: [BigInt(i + 1)] })));
  ...
  const handleJoin = async (tournament) => {
    ...
    try {
      ...
      if (BigInt(allowance) < BigInt(tournament.entryFee)) {
        const approveHash = await writeContract(wagmiAdapter.wagmiConfig, { address: ARCADE_TOKEN_ADDRESS, abi: ERC20_ABI, functionName: "approve", args: [TOURNAMENT_ADDRESS, tournament.entryFee], gas: BigInt(100000) });
