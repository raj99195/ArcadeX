...
import {
  ...
  serverTimestamp, deleteDoc, doc, limit, where, getDocs
...
export default function Community() {
  ...
  useEffect(() => {
    const q = query(
      collection(db, "community", activeChannel, "messages"),
  ...
  // Random online count (UI only)
  useEffect(() => {
    setOnlineCount(Math.floor(Math.random() * 8) + 2);
  }, [activeChannel]);

  const shortAddr = (addr) => addr ? addr.slice(0, 6) + "..." + addr.slice(-4) : "";
  const displayName = (addr) => nftProfiles[addr?.toLowerCase()]?.username
    ? nftProfiles[addr.toLowerCase()].username + ".arcade"
    : shortAddr(addr);
  ...
  const sendMessage = async () => {
    ...
    setSending(true);
    try {
      await addDoc(collection(db, "community", activeChannel, "messages"), {
        text: input.trim(),
        address: address.toLowerCase(),
        avatarStyle: getActiveAvatarStyle(address),
  ...
  const deleteMessage = async (msgId) => {
    if (!isAdmin) return;
    await deleteDoc(doc(db, "community", activeChannel, "messages", msgId));
  };
    ...
    <div style={{
        ...
        {CHANNELS.map(ch => (
          <div key={ch.id} onClick={() => { setActiveChannel(ch.id); setShowSidebar(false); }}
            style={{
              ...
              background: activeChannel === ch.id ? "rgba(123,47,255,0.15)" : "transparent",
              border: activeChannel === ch.id ? `1px solid rgba(123,47,255,0.3)` : "1px solid transparent",
          ...
          >
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ fontSize: 12, color: activeChannel === ch.id ? P.purpleL : P.dim, fontFamily: P.raj, fontWeight: 700, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                # {ch.label}
              </div>
            </div>
      ...
      {isConnected && (
        <div style={{ padding: "10px 12px", borderTop: `1px solid ${P.border}`, display: "flex", alignItems: "center", gap: 8 }}>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ fontSize: 11, color: P.purpleL, fontFamily: P.raj, fontWeight: 700, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
              {displayName(address)}
            </div>
    ...
    <div style={{ minHeight: "calc(100vh - 54px)", background: P.bg, display: "flex", overflow: "hidden", position: "relative" }}>
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", minWidth: 0, height: "calc(100vh - 54px)" }}>
        ...
        <div style={{ padding: "12px 20px", borderBottom: `1px solid ${P.border}`, background: "rgba(13,11,26,0.8)", display: "flex", alignItems: "center", gap: 12, flexShrink: 0 }}>
          {isMobile && (
            <button onClick={() => setShowSidebar(true)} style={{ background: "rgba(123,47,255,0.1)", border: `1px solid ${P.border}`, borderRadius: 6, padding: "4px 8px", color: P.purpleL, cursor: "pointer", fontSize: 14 }}>☰</button>
          )}
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: "16px 20px", display: "flex", flexDirection: "column", gap: 2 }}>
          ...
          {messages.map((msg, i) => {
            ...
            const grouped = prevMsg?.address === msg.address && prevMsg?.createdAt &&
              msg.createdAt && (msg.createdAt.toDate() - prevMsg.createdAt.toDate()) < 120000;
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  {!grouped && (
                    <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 3 }}>
                      <span style={{ fontSize: 13, fontWeight: 700, color: isOwn ? P.purpleL : isAdminMsg ? P.gold : "#d4b8ff", fontFamily: P.raj }}>
                        {displayName(msg.address)}
                      </span>
                      {isAdminMsg && (
                        <span style={{ fontSize: 9, padding: "1px 6px", background: "rgba(255,183,0,0.12)", border: "1px solid rgba(255,183,0,0.25)", borderRadius: 3, color: P.gold, fontFamily: P.raj, fontWeight: 700 }}>ADMIN</span>
                      )}
                      <span style={{ fontSize: 10, color: P.dimMore, fontFamily: P.mono }}>{timeAgo(msg.createdAt)}</span>
                    </div>
                  )}
                  <div style={{ fontSize: 13, color: "#c4b8e0", fontFamily: "'Inter', sans-serif", lineHeight: 1.5, wordBreak: "break-word" }}>
                    {msg.text}
                  </div>
                </div>
        ...
        <div style={{ padding: "12px 20px 16px", borderTop: `1px solid ${P.border}`, background: "rgba(13,11,26,0.8)", flexShrink: 0 }}>
          {!isConnected ? (
            <div style={{ display: "flex", alignItems: "center", gap: 12, padding: "12px 16px", background: "rgba(123,47,255,0.06)", border: `1px solid ${P.border}`, borderRadius: 10 }}>
              ...
              <button onClick={() => open()} style={{ padding: "7px 16px", background: `linear-gradient(135deg, ${P.purple}, #5a1fd4)`, border: "none", borderRadius: 7, color: "#fff", fontSize: 12, fontWeight: 700, cursor: "pointer", fontFamily: P.raj, letterSpacing: "0.5px" }}>
                Connect
              </button>
            </div>
          ) : channel?.adminOnly && !isAdmin ? (
            <div style={{ padding: "12px 16px", background: "rgba(255,183,0,0.05)", border: "1px solid rgba(255,183,0,0.15)", borderRadius: 10, fontSize: 12, color: P.gold, fontFamily: P.raj, textAlign: "center" }}>
            ...
            <div style={{ display: "flex", gap: 10, alignItems: "flex-end" }}>
              ...
              <div style={{ flex: 1, position: "relative" }}>
                <textarea
                  ...
                  rows={1}
                  style={{
                    width: "100%", padding: "10px 14px", background: "rgba(123,47,255,0.07)",
                    border: `1px solid rgba(123,47,255,0.2)`, borderRadius: 10, color: "#d4b8ff",
                    fontSize: 13, fontFamily: "'Inter', sans-serif", resize: "none",
              ...
              <button onClick={sendMessage} disabled={!input.trim() || sending}
                style={{
                  width: 38, height: 38, borderRadius: 9, border: "none", cursor: !input.trim() || sending ? "not-allowed" : "pointer",
                  background: !input.trim() || sending ? "rgba(123,47,255,0.2)" : `linear-gradient(135deg, ${P.purple}, #5a1fd4)`,
                  color: !input.trim() || sending ? P.dimMore : "#fff", fontSize: 16, flexShrink: 0,
